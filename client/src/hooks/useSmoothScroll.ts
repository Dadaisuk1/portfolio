...
export function useSmoothScroll(enabled: boolean) {
  useEffect(() => {
    ...
    // Lenis disabled wholesale to isolate a second-scrollbar bug — with it
    // off, the page falls back to plain native scroll (scrollToTarget/
    // scrollToTop in lib/smoothScroll.ts already have native fallbacks for
    // when lenis is null, so nav links / back-to-top keep working).
    // ScrollTrigger still works standalone off native scroll without a
    // Lenis proxy, so the section reveal animations are unaffected.
    ...
    //   autoRaf: true,
    //   // Lenis walks up from the wheel target looking for a nested
    //   // overflow:auto/scroll ancestor and, if it's actually scrollable, hands
    //   // wheel control back to native browser scroll instead of animating it.
