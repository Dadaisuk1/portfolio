...
export function HalftoneReveal({
  ...
  useEffect(() => {
    ...
    // Resizing the canvas's actual pixel buffer (not just its CSS box) clears it
    // to black immediately, per spec, ahead of the next repaint. The panel's
    // open/close transition animates layout width over ~380ms, which fires this
    // observer on nearly every frame — debounced, so mid-transition the canvas
    // just gets scaled by the browser like any other element (smooth, no clear)
    // and only reallocates its real resolution once the size has settled.
    ...
    const loop = (now: number) => {
      ...
      (uniforms.uMouse.value as number[])[0] = m.sx;
      (uniforms.uMouse.value as number[])[1] = m.sy;
